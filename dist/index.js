...
async function mouseData(data) {
    ...
    if (['080000', '090000', '0a0000'].includes(dataHex)) {
        child_process_1.default.exec(`aplay ${getMouseSoundFile(dataHex === '080000' ? 'up' : 'down')}`);
    }
}
...
async function keyboardData(data) {
    ...
    if ([1, 0].includes(action)) {
        child_process_1.default.exec(`aplay ${getKeyboardSoundFile(key, !action ? 'down' : 'up')}`, (error) => error && console.log(error));
    }
}
async function run() {
    ...
    if (mousePath) {
        const mouseStream = fs_1.default.createReadStream(mousePath);
        mouseStream.on('data', mouseData);
    }
    if (keyboardPath) {
        const keyboardStream = fs_1.default.createReadStream(keyboardPath);
